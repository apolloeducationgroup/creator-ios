...
//formURL = [formURL stringByAppendingFormat:@"%@",[paramString stringByAddingPercentEscapesUsingEncoding:NSASCIIStringEncoding]];
        ...
        NSString * parm=(__bridge NSString *)urlString;
        
        formURL=[formURL stringByAppendingString:parm];
        NSLog(@"riyaz url parm for pnuser %@",formURL);
    }
    return formURL;
}
...
+ (NSString*) postAuthTokenWithApplication : (ZCApplication*) _application {
    ...
    ZOHOUser *user = [ZOHOUser userObject];
    NSString *urlString = [NSString stringWithFormat:@"zcRefValue=true&authtoken=%@&scope=creatorapi&zc_ownername=%@",[user authToken],[_application appOwner]];
    return urlString;
}
